...
The main aim of the current program is to create the given number of emails with the customized set of visualizations of indicators also presented in the CEHS tool.
...
* Creation of the visualizations and captions
...
The output of the first part of the program is a json file and four visualizations for each indicator. The first three visualizations are: a scatter plot with an overview of the indicator at county level, a bar chart with facilities contribution and a reporting scatter plot. The fourth visualization is an overview scatter for an indicator, but at country level. The json file `titles.json`  contains figure titles, which are implemented in emails as figure captions.
...
### HOW TO RUN THE PROGRAM <a name="first"/>
...
1. "extract" - creates and prints the visualizations to the predefined folders;
...
### HOW TO CHANGE RECIPIENTS <a name="second"/>
...
### HOW TO CHANGE A SENDER <a name="thirdnull"/>
...
### HOW TO CHOOSE THE DATE <a name="third"/>
...
### HOW TO ADD OR DELETE INDICATORS OR DISTRICTS <a name="fourth"/>
...
### HOW TO ALTER THE EMAIL TEMPLATE <a name="fifth"/>
...
#### ADDING FIGURES <a name="fifthpointone"/>
...
#### ADDING CAPTIONS <a name="fifthpointtwo"/>
...
### HOW TO ALTER CAPTIONS <a name="sixth"/>
...
### DICTIONARY FOR DEVELOPERS <a name="seventh"/>

#### Table of content of the program

* [config](#config) - configuration files (.json)
* [data](#data) - output folder, where visualizations and captions (in json format) are stored
* [dataset](#dataset) - data transformation
* [emails](#emails) - creation and compilation emails
* [extract](#extract) - creation of visualizations and captions
...
[email_recipients.json](config/email_recipients.json) contains a list of dictionaries, where recipients' email addresses, names and districts are specified. Any changes in this json affect only dispatch of emails, but not visualizations or list of districts (for more, see [here](#second))
[email_template.json](config/email_template.json) is the template in html-format. Brings together visualizations, captions and aggs (see [emails](#emails) and [How to alter an email template](#fifth) for more)

##### DATA <a name="data"/>

Output directory.
The folder [viz](data/viz) includes [extracted](#extract) visualizations and captions, sorted by district -> reporting date -> indicator.
...
The backbone of the visualizations' printing is based in the [extract](extract) folder.
...
### Setting up the scheduler <a name="ninth"/>
...
### Adding a year to the overview graph <a name="tenth"/>
